...
const Dock = ({ windowsState, setWindowsState, setMinimizedWindows }) => {
  ...
  const { registerDockIcon } = useMinimizeAnimation();
  ...
  const handleMouseMove = (e) => {
    Object.values(dockRefs.current).forEach((icon) => {
      ...
      let scale = maxScale - (distance / influenceRadius) * (maxScale - minScale);
  ...
  return (
    <footer onMouseMove={handleMouseMove} onMouseLeave={resetDock}
      className={`dock ${isAnyWindowOpen ? 'has-active' : ''}`}>
        <div 
        ref={(el) => { dockRefs.current.github = el; registerDockIcon?.("github", el); }}
          onClick={() => handleDockClick('github')}
          className='icon github' title="GitHub" role="button"><img src="/doc-icons/github.svg" alt="" />
          {windowsState.github && <span className="active-dot"></span>}
          </div>

        <div
        ref={(el) => { dockRefs.current.note = el; registerDockIcon?.("note", el); }}
          onClick={() => handleDockClick('note')} 
          className='icon note' title="Notes" role="button"><img src="/doc-icons/note.svg" alt="" />
          {windowsState.note && <span className="active-dot"></span>}
          </div>

        <div
        ref={(el) => { dockRefs.current.resume = el; registerDockIcon?.("resume", el); }}
          onClick={() => handleDockClick('resume')}
          className='icon pdf' title="Resume" role="button"><img src="/doc-icons/pdf.svg" alt="" />
        ...
        <div
        ref={(el) => { dockRefs.current.spotify = el; registerDockIcon?.("spotify", el); }}
          onClick={() => handleDockClick('spotify')}
          className='icon spotify' title="Spotify" role="button"><img src="/doc-icons/spotify.svg" alt="" />
        ...
        ref={el => (dockRefs.current.mail = el)}
         onClick={()=>{window.open("mailto:lalitasolanki2577@gmail.com","_blank")}}
          className='icon mail' title="Mail" role="button"><img src="/doc-icons/mail.svg" alt="" /></div>

        <div
        ref={el => (dockRefs.current.linkedin = el)}
          onClick={()=>{window.open("https://www.linkedin.com/in/simerdeep-singh-gandhi/","_blank")}}
          className='icon linkedin' title="LinkedIn" role="button"><img src="/doc-icons/linkedin_v2.svg" alt="" /></div>

        <div
        ref={(el) => { dockRefs.current.cli = el; registerDockIcon?.("cli", el); }}
         onClick={() => handleDockClick('cli')}
          className='icon cli' title="Terminal" role="button"><img src="/doc-icons/cli.svg" alt="" />
